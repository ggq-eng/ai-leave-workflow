...
> **分类**：来源待确认 ｜ **文件数**：5 ｜ **仓库目录**：`ai-leave-workflow`
...
```bash
# 用户级（推荐）
cp -r . ~/.workbuddy/skills/ai-leave-workflow

# 或项目级
cp -r . <你的项目>/.workbuddy/skills/ai-leave-workflow
```
...
- **`leave_config.json`**：需要填写 `corpid`、`secret`、`trusted_ip`、`template_id`、`use_smart_table`、`default_creator`、`default_creator_name`、`default_approver`、`default_approver_name`、`note`
